...
def export_survey_responses_csv(db: Session, survey_id: UUID, current_user: User):
    ...
    for index, question in enumerate(questions, start=1):
        prefix = f"Q{index}"
        question_headers.extend([f"{prefix}_Order", f"{prefix}_Type", f"{prefix}_Title"])
        if question.type == "arrow":
            question_headers.extend([f"{prefix}_Angle", f"{prefix}_Deviation"])
        else:
            question_headers.append(f"{prefix}_Answer")
        question_headers.extend([f"{prefix}_Answered_At", f"{prefix}_Response_Time_Sec"])
    ...
    for attempt in attempts:
        ...
        for index, question in enumerate(questions, start=1):
            ...
            if question.type == "mcq":
                ...
                for answer in non_empty_answers:
                    if answer.user_angle is not None:
                        angle_values.append(_format_integer(answer.user_angle))

                    if answer.angle_deviation is not None:
                        deviation_values.append(_format_integer(answer.angle_deviation))
                    elif answer.user_angle is not None and correct_angle is not None:
                        computed_deviation = _compute_arrow_deviation(answer.user_angle, correct_angle)
                        if computed_deviation is not None:
                            deviation_values.append(_format_integer(computed_deviation))
            ...
            if question.type == "arrow":
                row_data[f"{prefix}_Angle"] = _to_csv_value(angle_text)
                row_data[f"{prefix}_Deviation"] = _to_csv_value(deviation_text)
            else:
